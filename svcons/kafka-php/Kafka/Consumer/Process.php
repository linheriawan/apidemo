...
class Process
{
    ...
    protected function processRequest($data, $fd)
    {
        ...
        case \Kafka\Protocol\Protocol::FETCH_REQUEST:
            ...
            // foreach($result["topics"] as $kt=>$t){
            //     foreach($t["partitions"] as $kp=>$p){
            //         $result["topics"][$kt]["partitions"][$kp]["highwaterMarkOffset"]=1;
            //         $result["topics"][$kt]["partitions"][$kp]["messages"]=[$p["messages"][count($p["messages"])-1]];
            //     }
            // }
            
            //var_dump("fetch:".json_encode($data)."\nres:".json_encode($result),$fd);
